...
async def warm_up_llm(show_model_warning: bool = True) -> None:
    ...
    try:
        ...
        # --- Arès modification (Apache-2.0 §4: file changed) ---------------
        # Arès is model-agnostic and welcomes any LLM (local or hosted), so the
        # upstream "not a recommended frontier model" quality warning is removed.
        # In its place we surface a single ethical-use disclaimer.
        if raw_model:
            logger.info("Arès engine model: %s", raw_model)
        ethics = Text()
        ethics.append("ARÈS — ETHICAL USE ONLY", style="bold red")
        ethics.append("\n\n", style="white")
        ethics.append(
            "Arès is an autonomous offensive-security tool for AUTHORIZED testing "
            "only — on systems you own or have explicit written permission to test.\n"
            "By continuing you confirm you hold that authorization. You alone are "
            "responsible for how you use Arès.\n"
            "The authors accept NO liability for any misuse, or for any harm caused "
            "by use outside its intended lawful and ethical purpose.",
            style="white",
        )
        ethics.append("\n\nWorks with any LLM — local (Ollama, etc.) or hosted.",
                      style="dim white")
        console.print(
            Panel(
                ethics,
                title="[bold white]ARÈS",
                title_align="left",
                border_style="red",
                padding=(1, 2),
            ),
