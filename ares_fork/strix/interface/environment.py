...
def pull_docker_image() -> None:
    ...
    with console.status("[bold cyan]Downloading image layers...", spinner="dots") as status:
        try:
            ...
            # Arès: a local-only image (e.g. re-tagged ares-sandbox) 404s on pull.
            # If it's present locally, that's fine — proceed instead of aborting.
            if image_exists(client, image):
                logger.info("Pull unavailable but image present locally: %s", image)
                console.print(f"[dim yellow]Using local image {image} (no registry pull)[/]\n")
                return
            console.print()
            error_text = Text()
