...
# Arès: cloud-provider prefixes routed by the cascade "escalate" tier must NOT
# inherit the local (Ollama) api_base — they call their own hosted endpoint.
_ARES_CLOUD_PREFIXES = ("anthropic/", "gemini/", "groq/", "mistral/",
                        "openrouter/", "cohere/", "vertex", "bedrock",
                        "azure/", "deepseek/", "xai/")


def _ares_is_cloud(model_name):
    return bool(model_name) and any(str(model_name).startswith(p)
                                    for p in _ARES_CLOUD_PREFIXES)
    ...
    def get_model(self, model_name: str | None) -> Model:
        ...
        if slug:
        ...
        elif _ares_is_cloud(model_name):
            # Escalate tier: strip the local api_base so litellm reaches the
            # real hosted provider (needs the provider's API key in the env).
            ...
            try:
                ...
                _ll = None
            _saved_env = _os.environ.pop("OPENAI_BASE_URL", None)
            _saved_ll = getattr(_ll, "api_base", None) if _ll else None
            if _ll is not None:
                _ll.api_base = None
            try:
                ...
                if _saved_env is not None:
                    _os.environ["OPENAI_BASE_URL"] = _saved_env
                if _ll is not None:
                    _ll.api_base = _saved_ll
        else:
            model = super().get_model(model_name)
