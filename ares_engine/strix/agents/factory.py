...
_ARES_LOCAL_REINFORCE = (
    "\n\n## ARÈS — MANDATORY DISCIPLINE (local model)\n"
    "- You MUST read every in-scope target file with the file tool BEFORE any "
    "conclusion. Never claim a file is a 'stub', 'empty', or 'has no code' "
    "without having actually read its contents in this session.\n"
    "- For each suspected issue, you MUST attempt at least one concrete test or "
    "PoC using your tools before deciding.\n"
    "- NEVER conclude 'no vulnerabilities' from reasoning alone. 'No findings' is "
    "only valid after you have read the code AND exercised the endpoints.\n"
    "- A finding is reported ONLY via create_vulnerability_report with a concrete "
    "reproduction. Do not end the scan while unread in-scope files remain."
)


def _ares_reinforce(instructions: str) -> str:
    import os
    if os.getenv("ARES_LOCAL_REINFORCE", "1").strip().lower() in ("1", "true", "on", "yes"):
        return instructions + _ARES_LOCAL_REINFORCE
    return instructions

def _ares_route_model(name: str, is_root: bool) -> "str | None":
    """Arès model-cascade router: pick an engine per agent role.

    OFF by default (ARES_CASCADE unset) -> returns None so every agent uses the
    single default model (upstream behaviour, fully backwards-compatible).

    When ARES_CASCADE is on, cheap breadth work (root coordination, recon,
    mapping, enumeration) runs on the fast *triage* model, while the calls that
    decide truth (validation, exploitation, PoC, reporting) run on the strong
    *validate* model. Both are local models on the same endpoint, so only the
    model name changes. See ares-cascade.yaml. (Apache-2.0 §4: Arès addition.)
    """
    import os
    if os.getenv("ARES_CASCADE", "").strip().lower() not in ("1", "true", "on", "yes"):
        return None
    triage = os.getenv("ARES_CASCADE_TRIAGE", "openai/ares-cascade-triage-qwen3-5")
    validate = os.getenv(
        "ARES_CASCADE_VALIDATE",
        "openai/ares-cascade-validate-qwen3-6-27b-obliterated",
    )
    n = (name or "").lower()
    strong_markers = ("valid", "exploit", "poc", "report", "confirm", "verif", "inject")
    # Root coordinates strategy + decides what to spawn — quality matters most
    # there, so the ROOT runs on the strong tier. Breadth children stay on triage.
    strong = is_root or any(m in n for m in strong_markers)
    escalate = os.getenv("ARES_CASCADE_ESCALATE", "").strip().lower() in ("1","true","on","yes")
    esc_model = os.getenv("ARES_CASCADE_ESCALATE_MODEL", "anthropic/claude-sonnet-5")
    # Escalate the highest-stakes agents (validation/exploit/report) to a hosted
    # frontier model when the operator opts in; otherwise stay on the local tiers.
    if strong and escalate and any(m in n for m in ("valid","exploit","report","confirm")):
        model = esc_model
    else:
        model = validate if strong else triage
    # Hosted escalate is a run-level choice for now (Strix forces one global
    # api_base), so per-agent local->hosted routing falls back to the local
    # strong tier. Set STRIX_LLM to a hosted model for a full hosted run.
    try:
        import logging
        logging.getLogger("strix.agents.factory").info(
            "Ares cascade: agent '%s' -> %s", name, model)
    except Exception:
        pass
    return model
...
def build_strix_agent(
    *,
    ...
    instructions = _ares_reinforce(instructions)
    ...
    return SandboxAgent(
        ...
        model=_ares_route_model(name, is_root),
        capabilities=[
            Filesystem(
