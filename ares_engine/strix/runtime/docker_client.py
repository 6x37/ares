...
def _apply_run_labels(create_kwargs: dict[str, Any]) -> None:
    ...
    if not isinstance(labels, dict):
        return
    labels["ares.managed"] = "true"
    labels["ares.run-id"] = run_id
    run_type = os.getenv("STRIX_RUN_TYPE")
    if run_type:
        labels["ares.run-type"] = run_type
...
def _ares_slug(text: str) -> str:
    import re as _re
    return _re.sub(r"[^a-z0-9]+", "-", str(text).lower()).strip("-") or "x"


def _ares_run_slug(session_id: Any = None) -> str:
    """Best available Arès run identifier: env -> report-state run name -> session."""
    rid = os.getenv("STRIX_RUN_ID")
    if not rid:
        try:
            from strix.report.state import get_global_report_state
            rs = get_global_report_state()
            rid = getattr(rs, "run_name", None) or (
                rs.run_record.get("run_name") if rs and hasattr(rs, "run_record") else None)
        except Exception:  # noqa: BLE001
            rid = None
    if not rid and session_id:
        rid = str(session_id)[:8]
    return _ares_slug(rid or "run")


def _ares_name_anonymous_volumes(mounts: Any, session_id: Any) -> Any:
    if not mounts:
        return mounts
    run = _ares_run_slug(session_id)
    for i, m in enumerate(mounts):
        try:
            if m.get("Type") == "volume" and not m.get("Source"):
                tgt = _ares_slug(m.get("Target", "v%d" % i))
                suffix = uuid.uuid4().hex[:4]  # uniqueness without a 64-hex hash
                m["Source"] = f"ares-{run}-{tgt}-{suffix}"
        except Exception:
            continue
    return mounts
    ...
    async def _create_container(
        ...
        *,
        ...
        if manifest is not None:
            ...
            if docker_mounts:
                docker_mounts = _ares_name_anonymous_volumes(docker_mounts, session_id)
        ...
        if "name" not in create_kwargs:
            create_kwargs["name"] = (
                f"ares-sandbox-{_ares_run_slug(session_id)}-{uuid.uuid4().hex[:4]}")
        ...
        # Arès fix: the installed openai-agents SDK (>=0.19) sets
        # entrypoint=["tail"], overriding the image's docker-entrypoint.sh — the
        # script that starts the in-sandbox caido-cli proxy. Without it Caido
        # never binds :48080 and bootstrap_caido times out. Restore the real
        # entrypoint and keep tail as the keep-alive command. (Apache-2.0 §4.)
        create_kwargs["entrypoint"] = ["docker-entrypoint.sh"]
